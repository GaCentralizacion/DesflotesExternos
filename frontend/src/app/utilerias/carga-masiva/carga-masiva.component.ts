...
export class CargaMasivaComponent implements OnInit {
    ...
    // tslint:disable-next-line: no-input-rename
    @Input('urlTemplate') urlTemplate: string;
    // tslint:disable-next-line: no-input-rename
    ...
    downloadTemplate(): void {
        if (this.urlTemplate !== undefined) {
            this.excelService.downloadTemplateToFileServer(this.urlTemplate, 'template');
        }
        else {
        ...
        if (column.tipo === type) {
            ...
            try {
                switch (column.tipo) {
                    ...
                    case TypeData.FLOAT:
                        if (obligatorio || valueExcel) {
                            const valInt = type === TypeData.NUMBER ? valueExcel + '.00' : valueExcel;
    ...
    receiveMessage($event: any): void {
        ...
        if ($event.recordsets[1].length > 0) {
            ...
            const arrayObligatorios: any = this.columnasTemplate.filter(col => col.obligatorio);

            const arrayNoObligatorios = this.columnasTemplate.filter(col => !col.obligatorio);
    ...
    changeSelectColumnaExcel($event: any, indexColumnaTemplate: number): void {
        if ($event !== undefined) {
            ...
            for (let index = 0; index < this.columnasExcel.length; index++) {
                if (this.columnasExcel[index].indexColumnaTemplate === indexColumnaTemplate &&
                    this.columnasExcel[index].seleccionado === true) {
                    this.columnasExcel[index].indexColumnaTemplate = null;
                    this.columnasExcel[index].seleccionado = false;
                    this.validateInputsRequired();
                    break;
    ...
    Guardar() {
        ...
        for (let i = 0; i < this.arrayProcesado.length; i++) {
            ...
            Object.keys(this.arrayProcesado[i]).forEach(key => {
                ...
                if (key !== 'Id') {
                    this.formDinamico.push({ propiedadDesc: key, valor: value, fechaCaducidad: null, id });
                } else {
                    id = value;
                    this.formDinamico.push({ propiedadDesc: key, valor: value, fechaCaducidad: null, id });
                }
